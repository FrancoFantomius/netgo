class SearchError(Exception):
    ...
    Every backend wraps its failures (transport errors, parsing
    problems, rate limiting) in ``SearchError`` subclasses, so caller
    code can catch this one class and handle all of them uniformly.
...
class SearchBlockedError(SearchError):
    """Raised when a search engine serves a captcha, interstitial or consent
    page instead of results.

    Both backends raise it: Google when it shows an interstitial/consent
    page, Bing when it shows a robot check. This usually means the
    requesting IP is rate-limited or flagged as a datacenter. It is raised
    instead of silently returning an empty list, so automation can detect
