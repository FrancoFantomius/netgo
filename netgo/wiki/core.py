"""Wikipedia core API: articles, search and page structure.

This module wraps the ``action=query`` endpoints of each language
edition of Wikipedia. Every function accepts ``lang`` to pick the
edition (the default is English), ``client`` to reuse a
:class:`~netgo.wiki.WikiClient` instance, plus ``timeout``/``delay``
matching the search backends.

Notes:
    - All text-based functions return plain text: ``explaintext`` strips
      markup, links and tables, so what you get is reader-friendly prose.
    - Pages that do not exist raise
      :class:`~netgo.wiki.WikiNotFoundError` (the API marks them
      ``missing``); nothing is silently returned.

Example:
    >>> from netgo.wiki import summary, paragraphs, geo_search
    >>> summary("Python (programming language)")
    'Python is a high-level, interpreted programming language...'
    >>> pars = paragraphs("Bread")
    >>> len(pars) > 1
    True
    >>> geo_search(41.9, 12.5, radius=10000, limit=2)[0].title
    'Vatican City'
"""

from __future__ import annotations

from concurrent.futures import ThreadPoolExecutor, as_completed
from typing import Iterable

from .client import WikiClient, _all_missing, _subject
from .models import Article, Category, GeoResult, Image, Link, Section, WikiResult


def _page_of(payload: dict, *, lang: str) -> dict:
    """Return the single page object out of a ``query.pages`` payload.

    Raises :class:`WikiNotFoundError` when the page is missing, so the
    callers do not repeat the check themselves.

    Example:
        >>> _page_of({"query": {"pages": {"1": {"title": "A", "missing": ""}}}}, lang="en")
        Traceback (most recent call last):
        ...
        netgo.wiki.errors.WikiNotFoundError: Not found: 'A' in 'en'
    """
    if _all_missing(payload):
        from .errors import WikiNotFoundError

        raise WikiNotFoundError(_subject(payload), lang=lang)
    pages = payload["query"]["pages"]
    return next(iter(pages.values()))


def _query(
    titles: str | None = None,
    *,
    lang: str = "en",
    client: WikiClient | None = None,
    timeout: int = 10,
    delay: float = 0.0,
    **props,
) -> dict:
    """Share the tiny plumbing used by the article-level wrappers."""
    client = client or WikiClient(lang=lang, timeout=timeout, delay=delay)
    params: dict[str, object] = {"action": "query"}
    if titles:
        params["titles"] = titles
    params.update(props)
    return client.api_call(params)


def search(
    query: str,
    lang: str = "en",
    limit: int = 10,
    namespace: int = 0,
    client: WikiClient | None = None,
    timeout: int = 10,
    delay: float = 0.0,
) -> list[WikiResult]:
    """Full-text search across a Wikipedia edition.

    Uses ``list=search`` (the same backend that powers MediaWiki's own
    search box) and returns the ranked hits with their snippet, size,
    word count and last-edit timestamp.

    Args:
        query: The text to search for.
        lang: Language code of the Wikipedia edition.
        limit: Maximum number of hits (MediaWiki caps at 50 per call).
        namespace: Namespace to search (0 = articles).
        client: Optional :class:`~netgo.wiki.WikiClient` to reuse.
        timeout: Request timeout in seconds.
        delay: Seconds to sleep before the request (rate limiting).

    Returns:
        A list of :class:`WikiResult`, ranked by the engine.

    Example:
        >>> from netgo.wiki import search
        >>> hits = search("deep learning", limit=3)
        >>> hits[0].title
        'Deep learning'
    """
    payload = _query(
        lang=lang,
        client=client,
        timeout=timeout,
        delay=delay,
        list="search",
        srsearch=query,
        srlimit=limit,
        srnamespace=namespace,
    )
    entries = payload["query"].get("search", [])
    out = []
    for e in entries:
        out.append(
            WikiResult(
                title=e.get("title", ""),
                pageid=e.get("pageid", 0),
                snippet=e.get("snippet", ""),
                wordcount=e.get("wordcount", 0),
                size=e.get("size", 0),
                timestamp=e.get("timestamp", ""),
                ns=e.get("ns", 0),
            )
        )
    return out


def summary(
    title: str,
    lang: str = "en",
    sentences: int = 5,
    client: WikiClient | None = None,
    timeout: int = 10,
    delay: float = 0.0,
) -> str:
    """Return the lead/intro of an article as plain text.

    Fetches the introductory paragraphs only (``exintro``), with markup
    stripped, so the result reads like the article's opening blurb.

    Args:
        title: The article title (normalized by the API on request).
        lang: Language code of the Wikipedia edition.
        sentences: Maximum number of sentences to keep.
        client: Optional :class:`~netgo.wiki.WikiClient` to reuse.
        timeout: Request timeout in seconds.
        delay: Seconds to sleep before the request (rate limiting).

    Returns:
        The article's lead paragraph(s) as a string.

    Raises:
        ~netgo.wiki.WikiNotFoundError: If the article does not exist.

    Example:
        >>> from netgo.wiki import summary
        >>> s = summary("Python (programming language)", sentences=1)
        >>> s.startswith("Python is")
        True
    """
    payload = _query(
        title,
        lang=lang,
        client=client,
        timeout=timeout,
        delay=delay,
        prop="extracts",
        exintro=True,
        explaintext=True,
        exsentences=sentences,
    )
    return _page_of(payload, lang=lang).get("extract", "")


def paragraphs(
    title: str,
    lang: str = "en",
    client: WikiClient | None = None,
    timeout: int = 10,
    delay: float = 0.0,
) -> list[str]:
    """Return the full body of an article split into paragraphs.

    Fetches the whole page as plain text and splits it on blank lines,
    so each item is one prose paragraph, ready for display or NLP
    downstream of this library.

    Args:
        title: The article title.
        lang: Language code of the Wikipedia edition.
        client: Optional :class:`~netgo.wiki.WikiClient` to reuse.
        timeout: Request timeout in seconds.
        delay: Seconds to sleep before the request (rate limiting).

    Returns:
        A list of paragraph strings; the first is the lead.

    Raises:
        ~netgo.wiki.WikiNotFoundError: If the article does not exist.

    Example:
        >>> from netgo.wiki import paragraphs
        >>> pars = paragraphs("Bread")
        >>> bool(pars[0].strip())
        True
    """
    payload = _query(
        title,
        lang=lang,
        client=client,
        timeout=timeout,
        delay=delay,
        prop="extracts",
        explaintext=True,
    )
    text = _page_of(payload, lang=lang).get("extract", "")
    return [p.strip() for p in text.split("\n\n") if p.strip()]


def sections(
    title: str,
    lang: str = "en",
    client: WikiClient | None = None,
    timeout: int = 10,
    delay: float = 0.0,
) -> list[Section]:
    """Return the table of contents (section outline) of an article.

    Uses ``action=parse&prop=sections`` so each heading of the article
    becomes a :class:`Section` with its index and nesting level.

    Args:
        title: The article title.
        lang: Language code of the Wikipedia edition.
        client: Optional :class:`~netgo.wiki.WikiClient` to reuse.
        timeout: Request timeout in seconds.
        delay: Seconds to sleep before the request (rate limiting).

    Returns:
        A list of :class:`Section`, in document order.

    Raises:
        ~netgo.wiki.WikiNotFoundError: If the article does not exist.

    Example:
        >>> from netgo.wiki import sections
        >>> toc = sections("Python (programming language)")
        >>> any(s.title == "History" for s in toc)
        True
    """
    client = client or WikiClient(lang=lang, timeout=timeout, delay=delay)
    payload = client.api_call(
        {"action": "parse", "page": title, "prop": "sections"}
    )
    parsed = payload.get("parse")
    if not parsed:
        from .errors import WikiNotFoundError

        raise WikiNotFoundError(title, lang=lang)
    out = []
    for s in parsed.get("sections", []):
        out.append(
            Section(
                index=s.get("index", ""),
                title=s.get("line", ""),
                level=int(s.get("level", 1) or 1),
            )
        )
    return out


def links(
    title: str,
    lang: str = "en",
    limit: int = 50,
    client: WikiClient | None = None,
    timeout: int = 10,
    delay: float = 0.0,
) -> list[Link]:
    """Return the wikilinks that leave an article.

    Args:
        title: The article title.
        lang: Language code of the Wikipedia edition.
        limit: Maximum number of links to fetch.
        client: Optional :class:`~netgo.wiki.WikiClient` to reuse.
        timeout: Request timeout in seconds.
        delay: Seconds to sleep before the request (rate limiting).

    Returns:
        A list of :class:`Link`.

    Raises:
        ~netgo.wiki.WikiNotFoundError: If the article does not exist.

    Example:
        >>> from netgo.wiki import links
        >>> out = links("Python (programming language)", limit=5)
        >>> out[0].ns
        0
    """
    payload = _query(
        title,
        lang=lang,
        client=client,
        timeout=timeout,
        delay=delay,
        prop="links",
        pllimit=min(limit, 500),
    )
    page = _page_of(payload, lang=lang)
    return [Link(title=l.get("title", ""), ns=l.get("ns", 0)) for l in page.get("links", [])]


def backlinks(
    title: str,
    lang: str = "en",
    limit: int = 50,
    client: WikiClient | None = None,
    timeout: int = 10,
    delay: float = 0.0,
) -> list[Link]:
    """Return the articles that link *to* a given page.

    "What links here" — a lightweight cross-reference useful for graph
    building and recommending related articles.

    Args:
        title: The page to look up.
        lang: Language code of the Wikipedia edition.
        limit: Maximum number of backlinks to fetch.
        client: Optional :class:`~netgo.wiki.WikiClient` to reuse.
        timeout: Request timeout in seconds.
        delay: Seconds to sleep before the request (rate limiting).

    Returns:
        A list of :class:`Link`.

    Example:
        >>> from netgo.wiki import backlinks
        >>> out = backlinks("Python (programming language)", limit=5)
        >>> bool(out)
        True
    """
    client = client or WikiClient(lang=lang, timeout=timeout, delay=delay)
    payload = client.api_call(
        {
            "action": "query",
            "list": "backlinks",
            "bltitle": title,
            "bllimit": min(limit, 500),
        }
    )
    return [
        Link(title=b.get("title", ""), ns=b.get("ns", 0))
        for b in payload["query"].get("backlinks", [])
    ]


def categories(
    title: str,
    lang: str = "en",
    limit: int = 50,
    client: WikiClient | None = None,
    timeout: int = 10,
    delay: float = 0.0,
) -> list[Category]:
    """Return the categories an article belongs to.

    Args:
        title: The article title.
        lang: Language code of the Wikipedia edition.
        limit: Maximum number of categories to fetch.
        client: Optional :class:`~netgo.wiki.WikiClient` to reuse.
        timeout: Request timeout in seconds.
        delay: Seconds to sleep before the request (rate limiting).

    Returns:
        A list of :class:`Category`.

    Raises:
        ~netgo.wiki.WikiNotFoundError: If the article does not exist.

    Example:
        >>> from netgo.wiki import categories
        >>> cats = categories("Python (programming language)")
        >>> any("Programming languages" in c.title for c in cats)
        True
    """
    payload = _query(
        title,
        lang=lang,
        client=client,
        timeout=timeout,
        delay=delay,
        prop="categories",
        cllimit=min(limit, 500),
    )
    page = _page_of(payload, lang=lang)
    return [
        Category(title=c.get("title", ""), ns=c.get("ns", 14), hidden=bool(c.get("hidden")))
        for c in page.get("categories", [])
    ]


def images(
    title: str,
    lang: str = "en",
    limit: int = 50,
    client: WikiClient | None = None,
    timeout: int = 10,
    delay: float = 0.0,
) -> list[Image]:
    """Return the ``File:`` media embedded in an article.

    Only titles are resolved here; for direct URLs, thumbnails and
    licence metadata use :func:`netgo.wiki.file_info` on each result.

    Args:
        title: The article title.
        lang: Language code of the Wikipedia edition.
        limit: Maximum number of images to fetch.
        client: Optional :class:`~netgo.wiki.WikiClient` to reuse.
        timeout: Request timeout in seconds.
        delay: Seconds to sleep before the request (rate limiting).

    Returns:
        A list of :class:`Image` with their ``File:`` titles.

    Raises:
        ~netgo.wiki.WikiNotFoundError: If the article does not exist.

    Example:
        >>> from netgo.wiki import images
        >>> imgs = images("Python (programming language)")
        >>> imgs[0].title.startswith("File:")
        True
    """
    payload = _query(
        title,
        lang=lang,
        client=client,
        timeout=timeout,
        delay=delay,
        prop="images",
        imlimit=min(limit, 500),
    )
    page = _page_of(payload, lang=lang)
    return [Image(title=i.get("title", "")) for i in page.get("images", [])]


def pageinfo(
    title: str,
    lang: str = "en",
    client: WikiClient | None = None,
    timeout: int = 10,
    delay: float = 0.0,
) -> dict:
    """Return basic metadata about an article.

    Includes the canonical URL, display title, last touched timestamp
    and pageid, exposed as a plain dict since the shape varies a lot.

    Args:
        title: The article title.
        lang: Language code of the Wikipedia edition.
        client: Optional :class:`~netgo.wiki.WikiClient` to reuse.
        timeout: Request timeout in seconds.
        delay: Seconds to sleep before the request (rate limiting).

    Returns:
        A dict with ``pageid``, ``title``, ``fullurl``, ``displaytitle``.

    Raises:
        ~netgo.wiki.WikiNotFoundError: If the article does not exist.

    Example:
        >>> from netgo.wiki import pageinfo
        >>> info = pageinfo("Bread")
        >>> info["fullurl"].endswith("/Bread")
        True
    """
    payload = _query(
        title,
        lang=lang,
        client=client,
        timeout=timeout,
        delay=delay,
        prop="info",
        inprop="url|displaytitle",
    )
    return _page_of(payload, lang=lang)


def page(
    title: str,
    lang: str = "en",
    client: WikiClient | None = None,
    timeout: int = 10,
    delay: float = 0.0,
) -> Article:
    """Fetch an article's structure in one aggregated object.

    Combines the intro summary, the section outline, the outgoing
    links, the categories and the image list in a single
    :class:`Article` using one API round-trip per property available in
    ``prop``.

    Args:
        title: The article title.
        lang: Language code of the Wikipedia edition.
        client: Optional :class:`~netgo.wiki.WikiClient` to reuse.
        timeout: Request timeout in seconds.
        delay: Seconds to sleep before the request (rate limiting).

    Returns:
        A populated :class:`Article`.

    Raises:
        ~netgo.wiki.WikiNotFoundError: If the article does not exist.

    Example:
        >>> from netgo.wiki import page
        >>> art = page("Bread")
        >>> bool(art.summary)
        True
    """
    payload = _query(
        title,
        lang=lang,
        client=client,
        timeout=timeout,
        delay=delay,
        prop="extracts|sections|categories|images|info",
        exintro=True,
        explaintext=True,
        inprop="url",
        cllimit=50,
        imlimit=50,
    )
    p = _page_of(payload, lang=lang)
    info = p.get("info", {})
    return Article(
        title=p.get("title", title),
        pageid=p.get("pageid", 0),
        lang=lang,
        summary=p.get("extract", ""),
        url=info.get("fullurl", ""),
        sections=[
            Section(index=s.get("index", ""), title=s.get("line", ""), level=int(s.get("level", 1) or 1))
            for s in p.get("sections", [])
        ],
        categories=[
            Category(title=c.get("title", ""), ns=c.get("ns", 14), hidden=bool(c.get("hidden")))
            for c in p.get("categories", [])
        ],
        links=[Link(title=l.get("title", ""), ns=l.get("ns", 0)) for l in p.get("links", [])],
        images=[Image(title=i.get("title", "")) for i in p.get("images", [])],
    )


def random(
    lang: str = "en",
    limit: int = 1,
    namespace: int = 0,
    client: WikiClient | None = None,
    timeout: int = 10,
    delay: float = 0.0,
) -> list[WikiResult]:
    """Return random article titles from a Wikipedia edition.

    Handy for building "explore" features or sampling corpora. The API
    picks uniformly among the pages of the requested namespace.

    Args:
        lang: Language code of the Wikipedia edition.
        limit: Number of random titles to return.
        namespace: Namespace to sample from (0 = articles).
        client: Optional :class:`~netgo.wiki.WikiClient` to reuse.
        timeout: Request timeout in seconds.
        delay: Seconds to sleep before the request (rate limiting).

    Returns:
        A list of :class:`WikiResult` with only the title filled in.

    Example:
        >>> from netgo.wiki import random
        >>> hits = random(limit=2)
        >>> len(hits)
        2
    """
    client = client or WikiClient(lang=lang, timeout=timeout, delay=delay)
    payload = client.api_call(
        {
            "action": "query",
            "list": "random",
            "rnlimit": min(limit, 500),
            "rnnamespace": namespace,
        }
    )
    return [
        WikiResult(title=r.get("title", ""), pageid=r.get("id", 0), ns=r.get("ns", 0))
        for r in payload["query"].get("random", [])
    ]


def geo_search(
    lat: float,
    lon: float,
    radius: int = 1000,
    limit: int = 10,
    lang: str = "en",
    client: WikiClient | None = None,
    timeout: int = 10,
    delay: float = 0.0,
) -> list[GeoResult]:
    """Find articles located near a coordinate.

    Uses ``list=geosearch``: every hit carries its coordinates and the
    distance (in metres) from the queried point, so you can power
    "near me" features or location-aware recommendations.

    Args:
        lat: Latitude of the centre point.
        lon: Longitude of the centre point.
        radius: Search radius in metres.
        limit: Maximum number of hits.
        lang: Language code of the Wikipedia edition.
        client: Optional :class:`~netgo.wiki.WikiClient` to reuse.
        timeout: Request timeout in seconds.
        delay: Seconds to sleep before the request (rate limiting).

    Returns:
        A list of :class:`GeoResult`, nearest first.

    Example:
        >>> from netgo.wiki import geo_search
        >>> hits = geo_search(51.5074, -0.1278, radius=5000, limit=2)
        >>> hits[0].dist < 5000
        True
    """
    client = client or WikiClient(lang=lang, timeout=timeout, delay=delay)
    payload = client.api_call(
        {
            "action": "query",
            "list": "geosearch",
            "gscoord": f"{lat}|{lon}",
            "gsradius": radius,
            "gslimit": min(limit, 500),
        }
    )
    out = []
    for g in payload["query"].get("geosearch", []):
        out.append(
            GeoResult(
                title=g.get("title", ""),
                pageid=g.get("pageid", 0),
                ns=g.get("ns", 0),
                lat=g.get("lat", 0.0),
                lon=g.get("lon", 0.0),
                dist=g.get("dist", 0.0),
            )
        )
    return out


def search_many(
    queries: Iterable[str],
    *,
    lang: str = "en",
    max_workers: int = 4,
    **kwargs,
) -> dict[str, list[WikiResult]]:
    """Run Wikipedia full-text :func:`search` for several queries in parallel.

    Follows the same contract as :func:`netgo.search.search_many`: every
    query is executed in a thread pool; a failing query maps to an empty
    list and its exception is stored under ``"<query>_error"``.

    Args:
        queries: An iterable of search queries.
        lang: Language code of the Wikipedia edition.
        max_workers: Size of the thread pool.
        **kwargs: Forwarded to :func:`search` (e.g. ``limit``).

    Returns:
        A mapping of ``query -> list[WikiResult]``.

    Example:
        >>> from netgo.wiki import search_many
        >>> out = search_many(["cats", "dogs"], limit=3)
        >>> list(out)
        ['cats', 'dogs']
    """
    queries = list(queries)
    results: dict[str, list[WikiResult]] = {}
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        futures = {pool.submit(search, q, lang=lang, **kwargs): q for q in queries}
        for fut in as_completed(futures):
            q = futures[fut]
            try:
                results[q] = fut.result()
            except Exception as exc:  # noqa: BLE001
                results[q] = []
                results[q + "_error"] = exc  # type: ignore[assignment]
    return results


__all__ = [
    "search",
    "summary",
    "paragraphs",
    "sections",
    "links",
    "backlinks",
    "categories",
    "images",
    "pageinfo",
    "page",
    "random",
    "geo_search",
    "search_many",
]